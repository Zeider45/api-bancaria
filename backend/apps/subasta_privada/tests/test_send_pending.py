...
from apps.subasta_privada import services


class SendPendingSubastaTests(TestCase):
    @override_settings(
    ...
    def test_sends_empty_payload_when_no_pending(self, mock_send_transaction):
        ...
        result = services.send_pending_solicitudes(webhook_url="https://webhook.example.test")
        ...
        self.assertEqual(endpoint, "libro-ordenes-subasta")
    ...
    def test_returns_error_when_no_pending_and_no_ente_code(self, mock_send_transaction):
        result = services.send_pending_solicitudes(webhook_url=None)
